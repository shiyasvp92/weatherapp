...
export default class SunriseChart extends Component {
    ...
    drawChart() {
        ...
        svg.append("g")
            .attr("id", "sun")
            .attr("style", `transform: translate(${xScale(currentTime) - 32}px, ${yScale(currentTimeTick / 3) - 28}px)`);

        const sunPath = d3.select("g#sun");

        sunPath.append("circle")
            .attr("fill", currentLight === "light" ? "#FFE634" : "#CCCCCC")
            .attr("stroke", currentLight === "light" ? "#F3AF36" : "#666667")
            .attr("cx", "32px")
            .attr("cy", "32px")
            .attr("r", 8);
        ...
        sunPath.append("path")
            .attr("class", `sun-leg ${currentLight}`)
